...
async function readWebFilePreview(file: File): Promise<string | undefined> {
...
async function readWebFileBase64(file: File): Promise<string | undefined> {
  try {
...
function pickFileViaWebInput(): Promise<ChatAttachment | null> {
  ...
  return new Promise((resolve) => {
    ...
    const handleChange = async () => {
      ...
      const textPreview = await readWebFilePreview(file);
      resolve({
        id: createAttachmentId(),
        name: file.name,
        mimeType,
